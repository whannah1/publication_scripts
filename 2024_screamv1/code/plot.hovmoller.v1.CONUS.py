...
fig_file,fig_type = 'figs/hovmoller.v1.CONUS','png'


# first_day,num_days = 0,24*10*2

first_day,num_days =  0,40

# first_day,num_days =  0,5
# first_day,num_days =  5,5
# first_day,num_days = 10,5
# first_day,num_days = 15,5
...
for v in range(num_var):
   ...
   for c in range(num_case):
      ...
      if recalculate_hov :
         ...
         # for f in file_list: print(f); exit()
         #----------------------------------------------------------------------
         with dask.config.set(**{'array.slicing.split_large_chunks': True}):
            if case[c]=='IMERG':
               data_ds = xr.open_mfdataset( file_list, group='Grid')
               # data_ds = xr.open_mfdataset( file_list, combine='nested', concat_dim='time',  )
            else:
            ...
            if obs_flag[c]:
               ...
               #-------------------------------------------------------------------
               # apply mask
               mask = xr.ones_like(lat,dtype=bool).load()#.isel(time=0,drop=True).load()
               if 'lat1' in globals(): mask = np.logical_and( mask, (lat>=lat1))
               if 'lat2' in globals(): mask = np.logical_and( mask, (lat<=lat2))
               if 'lon1' in globals(): mask = np.logical_and( mask, (lon>=lon1))
               if 'lon2' in globals(): mask = np.logical_and( mask, (lon<=lon2))
               data = data.where( mask, drop=True)
               # lat  =  lat.where( mask, drop=True).values
               lon  =  lon.where( mask, drop=True).values
            ...
            time = data['time.year'].values*365    \
