...
src_dir  = os.getenv('HOME')+'/E3SM/E3SM_SRC1/' # branch => whannah/mmf/2022-coupled-historical-rebase
...
### run duration
if debug_mode:
   queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',10,0,'0:30:00'
else:
   continue_run = True
   # queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',32,0,'1:00:00'
   # queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',73,4,'2:00:00'
   queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',365,20-1,'6:00:00'
...
# compset,arch,num_nodes = 'WCYCL20TR-MMF1', 'GNUGPU',64
compset,arch,num_nodes = 'WCYCL20TR',      'GNUCPU',64
...
lnd_data_path = '/global/cfs/cdirs/e3sm/inputdata/lnd/clm2/surfdata_map'
...
if 'WCYCL20TR' in compset:
   ...
   ic_date = '1960-01-01'
   ic_path = f'/global/cfs/cdirs/m3312/whannah/INCITE2022-CPL/{ic_case}/'
...
if config : 
   ...
   #-------------------------------------------------------
   # copy the initialization data files - this is already done for PM runs
...
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
# if debug_mode and copy_rest:
#    if arch=='GNUGPU': run_dir = f'/pscratch/sd/w/whannah/e3sm_scratch/pm-gpu/{case}/run'
#    if arch=='GNUCPU': run_dir = f'/pscratch/sd/w/whannah/e3sm_scratch/pm-cpu/{case}/run'
#    # copy restart and pointer files for debug case
#    print(f'\n{clr.RED}WARNING: copying restart files and pointer files from{clr.END}: {rest_case}')
#    run_cmd(f'cp ~/E3SM/scratch*/{rest_case}/run/rpointer* {run_dir}/')
#    run_cmd(f'cp ~/E3SM/scratch*/{rest_case}/run/*.r.*{rest_date}* {run_dir}/')
#    run_cmd(f'cp ~/E3SM/scratch*/{rest_case}/run/*.rst.*{rest_date}* {run_dir}/')
#    run_cmd(f'./xmlchange RUN_STARTDATE={rest_date},START_TOD=0')
#    run_cmd(f'./xmlchange RUN_REFCASE=\'{rest_case}\'')
#    run_cmd(f'./xmlchange RUN_REFDATE={rest_date}')
#    print()
#---------------------------------------------------------------------------------------------------
# Build
#---------------------------------------------------------------------------------------------------
