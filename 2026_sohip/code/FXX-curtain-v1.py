...
path_len_km = 1200e3  # total path distance in meters (dist_km)
...
for v in range(num_var):
    ...
    for c in range(num_case):
        ...
        data.load()
        print('!!!')
        data_interp = interpolate_to_path( path_npts[c], len(data['lev']), path_ncells, data.values, 
                                           path_lat[c], path_lon[c], 
    ...
    for c in range(num_case):
        ...
        data = data_list[c]
        contour = ax.contour(path_coord[c], data['lev'].values, data.values, **img_kwargs)
