...
fig_type = 'png'
fig_file = 'figs/F02-clim-map-diff'
...
for v in range(num_var):
   ...
   for c in range(num_case):
      ...
      if plot_diff and c in diff_case :
         ...
         if var[v] in ['PRECT','PRECC','PRECL'] :  tres.cnLevels = np.arange(-40,40+ 8, 8)/1e1
         if var[v] in ['TGCLDLWP'] :               tres.cnLevels = np.arange(-50,50+10,10)/1e3
         ...
         if var[v]=='PRECT':    tres.lbTitleString = 'mm/day'
         if var[v]=='TGCLDLWP': tres.lbTitleString = 'kg/m2'
         if common_colorbar and c==num_case-1: tres.lbLabelBarOn = True
         ...
         if plot_diff and not add_diff: ipd = v*(num_case-1)+c if var_x_case else (c-1)*num_var+v

         # print(); print(f' ipd: {ipd}   {tres.lbLabelBarOn}')
...
if plot_diff and not add_diff: 
   layout = [num_var,num_case-1] if var_x_case else [num_case-1,num_var]
