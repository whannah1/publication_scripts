...
add_var('TS',name='Sfc Temperature')

#-------------------------------------------------------------------------------
# lat1,lat2 = -60,60

# byr1,byr2 = 1961,1990 # baseline for anomalies to match HadCRU

# htype,yr1,yr2 = 'ha',1950,1951 # for testing
# htype,yr1,yr2 = 'ha',1950,2014
# htype,yr1,yr2 = 'ha',1985,1986
htype,yr1,yr2 = 'ha',1985,2014
date_str = f'{yr1}-{yr2}'

# htype,yr1,mn1,yr2,mn2 = 'h0',2000,3,2001,2
# htype,yr1,mn1,yr2,mn2 = 'ha',1950,0,2014,12
...
for v in range(num_var):
   ...
   #-------------------------------------------------------------------------
   # scrip_ds = xr.open_dataset('grid_files/HadCRU_36x72_scrip.20241001.nc')
   # area = scrip_ds['grid_area'].rename({'grid_size':'ncol'})
   #----------------------------------------------------------------------
   # scrip_ds = xr.open_dataset('grid_files/ne30pg2_scrip.nc')
   # area = scrip_ds['grid_area'].rename({'grid_size':'ncol'})
   #----------------------------------------------------------------------
   # # get area from HadCRU data file
   # obs_root = '/global/cfs/cdirs/m3312/whannah/obs_data/HadCRU'
   # obs_file = f'{obs_root}/HadCRUT.5.0.1.0.analysis.anomalies.ensemble_mean.nc'
   # ds = xr.open_dataset(obs_file)
   # xlon, ylat = np.meshgrid(ds['latitude'],ds['longitude'])
   ...
   # area = xr.DataArray(area,dims=('longitude','latitude')).transpose()
   # lat1D = ds['latitude']
   # lon1D = ds['longitude']
   #----------------------------------------------------------------------
   # # get area from BEST data file
   obs_file = f'/global/cfs/cdirs/m3312/whannah/obs_data/BEST/Land_and_Ocean_LatLong1.nc'
   ds = xr.open_dataset(obs_file).rename({'latitude':'lat','longitude':'lon'})
   # calculate area
   xlon, ylat = np.meshgrid(ds['lat'],ds['lon'])
   R = hc.earth_radius(ylat)
   dlat = np.deg2rad(np.gradient(ylat, axis=0))
   dlon = np.deg2rad(np.gradient(xlon, axis=1))
   dy,dx = dlat * R , dlon * R * np.cos(np.deg2rad(ylat))
   area = np.absolute(dy*dx) / np.square(R) # calculate area and convert to steridians
   area = xr.DataArray(area,dims=('lon','lat')).transpose()
   #----------------------------------------------------------------------
   for c in range(num_case):
      print('    case: '+hc.tcolor.GREEN+case[c]+hc.tcolor.ENDC)
      #-------------------------------------------------------------------------
      tmp_file1 = f'data/map-TS.{case[c]}.{var[v]}.nc'
      # map_file = 'map_files/map_ne30pg2_to_36x72_traave_20241001.nc'
      # tmp_file2 = f'data/map-TS.{case[c]}.{var[v]}.remap_36x72.nc'
      map_file = 'map_files/map_ne30pg2_to_180x360_BEST_traave_20241001.nc'
      tmp_file2 = f'data/map-TS.{case[c]}.{var[v]}.remap_180x360.nc'
      if recalculate:
         #----------------------------------------------------------------------
         if case[c]=='ERA5':
            ...
            time = ds['time_bnds'].isel(bnds=0)
            month_length = time.dt.days_in_month
            month_length['time'] = time
            wgts = month_length.groupby("time.year") / month_length.groupby("time.year").sum()
            data = ds['tas_mean'] .resample(time='Y').mean(dim='time')
            time = ds['time.year'].resample(time='Y').mean(dim='time')
            #-------------------------------------------------------------------
            # subset in time
            for t,y in enumerate(time.values):
               if y==yr1: t_beg=t
               if y==yr2: t_end=t
            yr_beg = data['time'].isel(time=t_beg).values
            yr_end = data['time'].isel(time=t_end).values
            # print()
            # print(f'yr1    / yr2   : {yr1}    / {yr2}')
            # print(f't_beg  / t_end : {t_beg}  / {t_end}')
            # print(f'yr_beg / yr_end: {yr_beg} / {yr_end}')
            # print(); exit()
            data = data.isel(time=slice(t_beg,t_end+1))
            #-------------------------------------------------------------------
            ...
            for f in range(len(file_list_all)):
               ...
               # if yr>=byr1 and yr<=byr2: file_list2.append(file_list_all[f])
         ...
         #----------------------------------------------------------------------
         # Write to file 
         if os.path.isfile(tmp_file1) : os.remove(tmp_file1)
         tmp_ds = xr.Dataset()
         tmp_ds[var[v]] = data
         tmp_ds.to_netcdf(path=tmp_file1,mode='w')
         #----------------------------------------------------------------------
         # perform remap to obs grid
         if not obs_flag[c]:
            run_cmd(f'ncremap -m {map_file} -i {tmp_file1} -o {tmp_file2}')
      #-------------------------------------------------------------------------
      if     obs_flag[c]: tmp_file = tmp_file1
      if not obs_flag[c]: tmp_file = tmp_file2
   ...
   for c in range(num_case):
      ...
      # tres.cnLevels = np.arange(-120,120+20,20)
      ...
      if not plot_diff  or (plot_diff and add_diff) or (plot_diff and c==diff_base) : 
         ...
         hs.set_subtitles( wks, plot[ip], name[c], ctr_str, var_str[v], 
                           center_sub_string=f'mean: {glb_avg_list[c]:5.2f} C', 
      ...
      if plot_diff and c!=diff_base :
         ...
         hs.set_subtitles( wks, plot[ipd], name[c], ctr_str, f'{var_str[v]} (diff)', 
