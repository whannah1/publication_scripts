...
for c in range(num_case):
   ...
   for v in range(num_var) :
      ...
      for s in range(num_set):
         ...
         # tname = name[c]
         tmp_name = name[c]
         if tmp_name=='OBS' and var[v]=='TGCLDLWP': tmp_name = 'MAC'
         if tmp_name=='OBS' and var[v]=='PRECT'   : tmp_name = 'IMERG'

         print(f'  {name[c]}  {tmp_name}')

         hs.set_subtitles(wks, plot[ip], tmp_name, '', var_str[v], font_height=subtitle_font_height)
