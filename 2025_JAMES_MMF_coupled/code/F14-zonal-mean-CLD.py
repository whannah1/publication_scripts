...
# add_case('E3SM.2023-CO2-TEST-01.GNUGPU.ne30pg2_EC30to60E2r2.WCYCL1850-MMF1.2xCO2',n='2xCO2-1xCO2',c='springgreen3',p=tmp_path_co2_mmf,s=tmp_sub)
# add_case('E3SM.2023-CO2-TEST-01.GNUGPU.ne30pg2_EC30to60E2r2.WCYCL1850-MMF1.4xCO2',n='4xCO2-2xCO2',c='tomato'  ,p=tmp_path_co2_mmf,s=tmp_sub)
# add_case('E3SM.2023-CO2-TEST-01.GNUGPU.ne30pg2_EC30to60E2r2.WCYCL1850-MMF1.4xCO2',n='4xCO2-1xCO2',c='tomato'  ,p=tmp_path_co2_mmf,s=tmp_sub)
...
add_case('E3SM.2023-CO2-TEST-01.GNUGPU.ne30pg2_EC30to60E2r2.WCYCL1850-MMF1.4xCO2',n='E3SM-MMF 4xCO2-1xCO2',c='tomato'  , p=tmp_path_co2_mmf,s=tmp_sub)
add_case('v2.LR.piControl',                                                       n='E3SMv2 1xCO2',        c='lightskyblue',d=1,  p=tmp_path_co2_v2, s=tmp_sub)
add_case('v2.LR.abrupt-4xCO2_0101',                                               n='E3SMv2 4xCO2-1xCO2',  c='hotpink',     d=1,  p=tmp_path_co2_v2, s=tmp_sub)

#-------------------------------------------------------------------------------

fig_file,fig_type = 'figs/F14-zonal-mean-CLD','png'
tmp_file_head = 'data/zonal-mean-CLD'

add_var('CLDLIQ',None,name='Cld Liq')
add_var('CLDICE',None,name='Cld Ice')
add_var('CLOUD', None,name='Cld Frac')
...
plev = np.array([50,75,100,125,150,175,200,225,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950])
#-------------------------------------------------------------------------------
...
subtitle_font_height = 0.008 # 0.01
...
for v in range(num_var):
   ...
   for c in range(num_case):
      ...
      #-------------------------------------------------------------------------
      if var[v]=='CLDLIQ': bin_ds['bin_val'] = bin_ds['bin_val']*1e3
      if var[v]=='CLDICE': bin_ds['bin_val'] = bin_ds['bin_val']*1e3
      #-------------------------------------------------------------------------
      # if case[c]=='ERA5':
   ...
   if var[v]=='CLOUD' : clev_main = np.arange(0.04,0.4+0.04,0.04)
   if var[v]=='CLDLIQ': clev_main = np.arange(0.004,0.040+0.004,0.004)
   if var[v]=='CLDICE': clev_main = np.arange(0.002,0.020+0.002,0.002)


   if var[v]=='U'     : clev_colors = np.array( cmocean.cm.delta(np.linspace(0,1,256)) )
   if var[v]=='T'     : clev_colors = np.array( cmocean.cm.thermal(np.linspace(0,1,256)) )
   if var[v]=='Q'     : clev_colors = np.array( cmocean.cm.dense(np.linspace(0,1,256)) )
   if var[v]=='CLOUD' : clev_colors = np.array( cmocean.cm.dense(np.linspace(0,1,256)) )
   if var[v]=='CLDLIQ': clev_colors = np.array( cmocean.cm.dense(np.linspace(0,1,256)) )
   if var[v]=='CLDICE': clev_colors = np.array( cmocean.cm.dense(np.linspace(0,1,256)) )
   ...
   # clev_main = np.linspace(cmin,cmax,num=21)
   ...
   if plot_diff and num_case>1:
      ...
      if var[v]=='CLOUD' : dlev = np.arange(-0.13,0.13+0.02,0.02)
      if var[v]=='CLDLIQ': dlev = np.arange(-0.022,0.022+0.004,0.004)
      if var[v]=='CLDICE': dlev = np.arange(-0.022,0.022+0.004,0.004)
   ...
   for c in range(num_case):
      ...
      if plot_diff:
         ...
         tres.cnFillPalette = clev_colors
         # if c==0:
         #    baseline = tmp_data
         if c in [1,3]: 
            # tmp_data = tmp_data - baseline
            if c==1: tmp_data = tmp_data - data_list[0]
            if c==3: tmp_data = tmp_data - data_list[2]
            tres.cnLevels = dlev
      ...
      if var[v]=='CLOUD' : unit_str = ''
      if var[v]=='CLDLIQ': unit_str = '[g/kg]'
      if var[v]=='CLDICE': unit_str = '[g/kg]'

      rstr = f'{var_str[v]} {unit_str}' if not (plot_diff and c>=1) else f'{var_str[v]} (diff) {unit_str}'
      # cstr = f'{yr1}-{yr2}'
