...
name,cases,clr,git_hash = [],[],[],'efa06a'
ntask = 1350; tclr = 'red'
cases.append(f'E3SM.PGVAL-TIMING.ne30_ne30.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')        ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg2_ne30pg2.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg3_ne30pg3.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg4_ne30pg4.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
ntask = 2700; tclr = 'green'
cases.append(f'E3SM.PGVAL-TIMING.ne30_ne30.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')        ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg2_ne30pg2.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg3_ne30pg3.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg4_ne30pg4.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
ntask = 5400; tclr = 'blue'
cases.append(f'E3SM.PGVAL-TIMING.ne30_ne30.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')        ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg2_ne30pg2.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg3_ne30pg3.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
cases.append(f'E3SM.PGVAL-TIMING.ne30pg4_ne30pg4.F-EAM-AQP1.master-{git_hash}.ntask_{ntask}.nthrds_2')  ; clr.append(tclr)
for c in cases:
   if 'E3SM.PGVAL-TIMING.ne30_'         in c: name.append('ne30np4')
   if 'E3SM.PGVAL-TIMING.ne30pg2_'      in c: name.append('ne30pg2')
   if 'E3SM.PGVAL-TIMING.ne30pg3_'      in c: name.append('ne30pg3')
   if 'E3SM.PGVAL-TIMING.ne30pg4_'      in c: name.append('ne30pg4')
...
fig_file = 'figs/F15-aqua-timing-with-ideal'
...
res.tmYLValues = np.arange(0,50,2)
...
for param in param_list :
      ...
      for l,line in enumerate(msg[num_file:]) : 
         ...
         if data_tmp is not None:
            if num_sim_days[dcnt]>min_num_days:
               ...
               # dpos.append(float(c))
               if 'ne30_'    in case: dpos.append(float(0))
               if 'ne30pg2_' in case: dpos.append(float(1))
               if 'ne30pg3_' in case: dpos.append(float(2))
               if 'ne30pg4_' in case: dpos.append(float(3))
            dcnt += 1
   ...
   apx = []
   for c,case in enumerate(cases):
      if 'ne30_'    in case: apx.append(float(0))
      if 'ne30pg2_' in case: apx.append(float(1))
      if 'ne30pg3_' in case: apx.append(float(2))
      if 'ne30pg4_' in case: apx.append(float(3))
   apx = np.array(apx)
   ...
   #-------------------------------------------------------------------------------
   # estimate ideal scaling here so plot bounds are adjusted
   ...
   if add_ideal:
      ...
      # for c,case in enumerate(cases):
      #    found = False
      #    if 'pg2' in case: npg.append(2); found = True
      #    if 'pg3' in case: npg.append(3); found = True
      #    if 'pg4' in case: npg.append(4); found = True
      #    if not found: 
      #       npg.append(0)
      #       if ref_cnt==0: yref = apy[c] # assume first non-pg case is the np4 reference
      #    ref_cnt += 1

      y_ideal = []
      for c,case in enumerate(cases):
         npg = None
         if 'pg2' in case: npg = 2
         if 'pg3' in case: npg = 3
         if 'pg4' in case: npg = 4
         if npg is None: npg = 0
         if npg==0: 
            yref = apy[c]
            y_ideal.append(0)
            continue # skip np4 cases
         ne = 30
         # if 'conus' in case[c]: ne = 30
         np_ncol = 54*ne*ne+2
         pg_ncol = (ne*ne*6*npg*npg)
         if 'a:' in param :
            y_ideal.append( yref * pg_ncol / np_ncol )
         else:
            y_ideal.append( yref * np_ncol / pg_ncol )
         

   # print()
   # print(yref)
   # tnpg = 2 ; print( 48602 / (30*30*6*tnpg*tnpg) )
   # tnpg = 3 ; print( 48602 / (30*30*6*tnpg*tnpg) )
   # tnpg = 4 ; print( 48602 / (30*30*6*tnpg*tnpg) )
   # for c,case in enumerate(cases[1:]): print(f'  {y_ideal[c]:8.4f}  {(y_ideal[c]/yref):8.4f} ')
   # exit()
   #-------------------------------------------------------------------------------
   # plot timing data
   ...
   if add_ideal:
      res.trYMinF = min( np.min(py), np.min(y_ideal) )-0.1*dy
      res.trYMaxF = max( np.max(py), np.max(y_ideal) )+0.1*dy
   else:
   ...
   for c,case in enumerate(cases):
      ...
      # print the speedup of pg2 cases
      if 'ne30_' in case: baseline_sypd = apy[c]
      if 'ne30pg2_' in case: 
         ratio = apy[c] / baseline_sypd
         print(f'  {case}    speedup: {ratio:6.2f}')
   ...
   # add markers to show idealized performance scaling
   if add_ideal:
      # # add markers for ideal scaling
      # for c,case in enumerate(cases):
      #    # skip np4 cases
      #    if y_ideal[c]==0: continue
      #    pmres.gsMarkerColor       = clr[c]
      #    ngl.add_polymarker(wks, plot[len(plot)-1], apx[c], y_ideal[c], pmres)

      # add lines for ideal scaling
      c1,c2,lclr,cnt = [],[],[],0
      for c,case in enumerate(cases):
         found = False
         if y_ideal[c]==0 and cnt==0: found = True; c1.append(c+1)
         if y_ideal[c]==0 and cnt!=0: found = True; c1.append(c+1); c2.append(c-1)
         if c==len(cases)-1: c2.append(c)
         if found: cnt += 1; lclr.append(clr[c])
      for c in range(cnt):
         # pmres.gsDashPattern = 1
         pmres.gsLineThicknessF = 4
         pmres.gsLineColor = lclr[c]
         ngl.add_polyline(wks, plot[len(plot)-1], apx[c1[c]:c2[c]+1], y_ideal[c1[c]:c2[c]+1], pmres)
