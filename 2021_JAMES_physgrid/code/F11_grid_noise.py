...
lat1,lat2,lon1,lon2 = 10,50,60,110  # India
...
for v in range(num_var):
   ...
   for c in range(num_case):
      ...
      if write_file:
         ...
         use_remap_flag = use_remap or ('CESM' in case[c] and 'ne30' not in case[c])
   ...
   #       data_list[d] = data_list[d]*100. * 60.
   ...
   for c in range(num_case):
      ...
      if 'ne30_'    in case[c]: case_obj.grid = 'ne30np4'
      if 'ne30pg2_' in case[c]: case_obj.grid = 'ne30pg2'
      if 'ne30pg3_' in case[c]: case_obj.grid = 'ne30pg3'
      if 'ne30pg4_' in case[c]: case_obj.grid = 'ne30pg4'
      case_obj.set_coord_names(var[v])
      ...
      if 'OMEGA' in var[v]             : tres.cnLevels = np.linspace(-1,1,13)*0.6
      ...
      #-------------------------------------------------------------------------
      #-------------------------------------------------------------------------
      if 'ne30_' in case[c] and var[v]=='DYN_OMEGA':
         blank_idx = ip
         blank_res = ngl.Resources()
         blank_res.tmXTOn = False
         blank_res.tmXBOn = False
         blank_res.tmYLOn = False
         blank_res.tmYROn = False
         blank_res.pmTickMarkDisplayMode = "Never"
         plot[ip] = ngl.blank_plot(wks,blank_res)
...
labels 
for i in reversed(range(len(labels)-1)):
   if i <blank_idx: labels[i] = labels[i]
   if i==blank_idx: labels[i] = ''
   if i >blank_idx: labels[i] = labels[i-1]

pnl_res.nglPanelFigureStrings            = labels
