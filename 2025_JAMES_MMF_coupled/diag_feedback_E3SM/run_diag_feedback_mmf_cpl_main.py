...
if machine == 'compy':
    ...
    www_dir = "/lcrc/group/e3sm/public_html/diagnostic_output/ac.bharrop/E3SMv2/"
    run_dir = "/lcrc/group/e3sm/ac.bharrop/E3SMv2/ppe_ens/"
elif machine == 'perlmutter':
    www_dir = "/global/cfs/cdirs/e3sm/www/whannah/2023-CPL"
    run_dir = "/global/cfs/cdirs/m3312/whannah/2023-CPL"
    # run_dir = "/global/cfs/cdirs/m3312/beharrop/"


e3sm_version = 'mmf' # E3SM version

PreProcess = True  # True: prepare input data for feedback calculation, including regrid and reorganize data
#PreProcess = False
hasCOSP = False   # True: include COSP output 
...
#         rgr_map = '/home/ac.zender/data/maps/map_ne30pg2_to_cmip6_180x360_aave.20200201.nc'
    ...
    if machine == 'compy':
        ...
        # rgr_map = '/global/cfs/cdirs/e3sm/zender/maps/map_ne30pg2_to_cmip6_180x360_aave.20200201.nc'
        rgr_map = '/global/homes/w/whannah/maps/map_ne30pg2_to_90x180_traave.nc'
    elif machine == 'chrysalis':
        rgr_map = '/home/ac.zender/data/maps/map_ne30pg2_to_cmip6_180x360_aave.20200201.nc'

#if ppe_number is not None:
#    # set input directory 1 --- the directory before casename in the whole directory
#    run_dir     = run_dir + '/workdir.' + str(ppe_number) + '/'
#    datadir_in1 = run_dir
#    # set input directory 2 --- the directory after casename in the whole directory
#    datadir_in2 = 'archive/atm/hist/'
...
if machine == 'compy':
    ...
    RadKernel_dir = '/global/cfs/cdirs/mp193/www/qinyi/DATA/Huang_kernel_data/'
elif machine == 'chrysalis':
    #RadKernel_dir = '/home/ac.xzheng/E3SMv3_dev/utils/cloud_feedback/diag_feedback_E3SM/Huang_kernel_data/'
    RadKernel_dir = '/home/ac.bharrop/diag_feedback_e3sm/feedback_data/'
...
outdir_final = outdir_out + '/data/'
# set output figure directory
#diagfigdir = curdir+'/diagfigure/'
diagfigdir = outdir_out + '/diagfigure/'
...
for icase,case in enumerate(case_short):
    ...
    if PreProcess:
        ...
        # Part 1: regrid data
        print('run_id1 = ' + run_id1, 'run_id2 = ' + run_id2,
              'rgr_map = ' + rgr_map, 'yearS_CTL = ' + str(yearS_CTL),
              'yearE_CTL = ' + str(yearE_CTL),
              'yearS_P4K = ' + str(yearS_P4K),
              'yearE_P4K = ' + str(yearE_P4K),
              'datadir_in1 = ' + datadir_in1,
              'datadir_in2 = ' + datadir_in2,
              'outdir_out = ' + outdir_out,
              'comp = ' + comp)
...
if GetFigure:
    ...
    if machine == 'compy':
        ...
        datadir_CMIPs = '/global/cfs/cdirs/mp193/www/qinyi/DATA/'
    elif machine == 'chrysalis':
    ...
    # datadir_v2 = datadir+'/data/' # don't use local directory to find things
    datadir_v2 = outdir_out + '/data/'
    ...
    if machine in ['compy','perlmutter','chrysalis']:
        AP.make_dir(webdir)
