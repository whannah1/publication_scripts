...
fig_type = 'png'
fig_file = 'figs/F07-chx-occurrence'
...
for c in range(num_case):
   ...
   for v in range(num_var) :
      ...
      print(f'    var: {vstr}  {case_tmp_file:90}  ({ntime} days / {nyear} years)')
...
for n in range(num_plot):
   ...
   for v in range(num_var):
      ...
      if var_name=='TGCLDIWP': var_name = 'Ice Water Path'
      if var_name=='PRECT':    var_name = 'Precipitation'
      if var_name=='LHFLX':    var_name = 'Sfc Latent Heat Flux'
      if var_name=='T850':     var_name = '850mb Temperature'
      if var_name=='Q850':     var_name = '850mb Specific Humidity'
...
# for (n,l) in enumerate(lname):
#    if 'OBS' in lname[n]: 
#       lname[n] = lname[n].replace('OBS','GPM')
# xpos = 0.3 if add_diff_plot else 0.5
# ypos = 0.3 + (num_var-1)*0.1
# pid = ngl.legend_ndc(wks, len(name), lname, xpos, ypos, lgres)
