...
# add_case('E3SM.2023-SCIDAC.ne30pg2_EC30to60E2r2.AMIP.EF_0.35.CF_10.HD_0.50',n='E3SM L80',c='blue',p=tmp_path,s=tmp_sub)
add_case('E3SM.2023-SCIDAC.ne30pg2_EC30to60E2r2.AMIP.EF_0.60.CF_07.HD_1.35',n='E3SM L80',c='green',p=tmp_path,s=tmp_sub)
#-------------------------------------------------------------------------------
...
fig_file,fig_type = 'figs/FXX-QBO-amplitude-profile-distribution','png'

tmp_file_head = 'data/QBO-amplitude-profile-distribution'
...
segment_length,segment_stride = 12*10,12
...
for v in range(num_var):
   ...
   #    lev_list_list.append( data_avg['plev'].values/1e2 )
   #    amp_list_list.append( amp )
   #----------------------------------------------------------------------------
   # read the data
   #----------------------------------------------------------------------------
   for c in range(num_case):
      ...
      num_segments = int((num_files-segment_length)/segment_stride)
      amp_list      = []
      lev_list      = []
      for n in range(num_segments):
         t_beg = n*segment_stride
         t_end = t_beg+segment_length

         data_avg_segment = data_avg.isel(time=slice(t_beg,t_end))

         psd, amp = get_20to40month_fft_amplitude(data_avg_segment.values, 
                                                  data_avg_segment['lev'].values)

         lev_list.append( data_avg['lev'].values )
         amp_list.append( amp )

      lev_list_list.append( lev_list )
      amp_list_list.append( amp_list )
   ...
   for c in range( (num_case+1) if add_obs else num_case ):
      for amp in amp_list_list[c]:
         amp_max = np.max([amp_max,np.nanmax(amp)])
   #----------------------------------------------------------------------------
   # Create plot
   #----------------------------------------------------------------------------
   ...
   tres.trXMinF = 0
   # tres.trXMaxF = 25 # 
   tres.trXMaxF = amp_max
   tres.trYMinF = 1e0
   tres.trYMaxF = 1e2

   for c in range( (num_case+1) if add_obs else num_case ):

      amp_list = amp_list_list[c]
      lev_list = lev_list_list[c]
      ...
      for n in range(len(amp_list)):
         ...
         # print(amp_list[n])
         # print(lev_list[n])
         tplot = ngl.xy(wks, amp_list[n], lev_list[n], tres)

         max_tmp = np.nanmax(amp_list[n])
