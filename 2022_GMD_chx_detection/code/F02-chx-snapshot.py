...
fig_type = 'png'
fig_file = 'figs/F02-chx-snapshot'

lat1,lat2 = -30,30

htype,years,months,first_file,num_files = 'h1',[],[],365*4,31 ; num_files_obs = 31
...
for v in range(num_var):
   ...
   for c in range(num_case):
      ...
      if 'time' in data.dims : 
         ...
         data = data.mean(dim='time') # average all times
         # data = data.isel(time=0) # take first value for snapshot
   ...
   for c in range(num_case):
      ...
      #-------------------------------------------------------------------------
      # if var[v] in ['PRECT','PRECC']   : tres.cnLevels = np.arange(4,80+4,4)
